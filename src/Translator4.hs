...
translaterule (head, body) i m n = [commentRule (head, body), makeBodyRule i m body] ++ makeChoiceRules head i m n ++ makeFstars (head, body) i m n ++ [""]
...
makeChoiceRules :: [Atom] -> Int -> Int -> Int -> [String]
makeChoiceRules head i m n = zipWith (\ atom r -> makeChoiceRule atom head i m r) (take (n-1) head) [1..n-1] ++ [makeContrRule head i n, ""]

makeChoiceRule :: Atom -> [Atom] -> Int -> Int -> Int -> String
makeChoiceRule atom head i m r = "{" ++ showAtom atom ++ "} :- " ++ "body_" ++ show i ++ (if r > 1 then ", " else "") ++ concatsep (map (\s -> "not " ++ showAtom s) (take (r-1) head)) ", " ++ "."

makeContrRule :: [Atom] -> Int -> Int -> String
makeContrRule head i n = showAtom (head !! (n-1)) ++ " :- " ++ "body_" ++ show i ++ (if n > 1 then ", " else "") ++ concatsep (map (\s -> "not " ++ showAtom s) (take (n-1) head)) ", " ++ "."

makeFstars :: ([Atom],[Literal]) -> Int -> Int -> Int -> [String]
makeFstars (head, body) i m n = makeFstarHeadOD head i n ++ ["", makeFstarBodyAux i m body, ""]
...
makeFstarHeadOD :: [Atom] -> Int -> Int -> [String]
makeFstarHeadOD head i n = map (makerulebn10 head i) [1..n]
    where
        isFstarAtom atom = "isFstar(" ++ showAtom atom ++ ")"
        makerulebn10 head i r = isFstarAtom (head !! (r-1)) ++ " :- isTFstar(body_" ++ show i ++ "), " ++ concatsep (map isFstarAtom (take (r-1) head)) ", " ++ (if r > 1 then ", " else "") ++ "not " ++ showAtom (head !! (r-1)) ++ "."
...
translateLPOD rules = map (`makeRegRuleBN` m) regrules ++ (["" | not (null regrules)]) ++ makeODrules odrules m ns ++ [""] ++ makeTFstarMstar headlist bodylist m
    where
